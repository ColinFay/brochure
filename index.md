...
The goal of [brochure](https://github.com/colinfay/brochure) is to
provide a mechanism for creating natively multi-page
[shiny](https://shiny.posit.co/) applications, *i.e* that can serve
content on multiple endpoints.

**Disclaimer**: the way you will build app with
[brochure](https://github.com/colinfay/brochure) is different from the
way you usually build [shiny](https://shiny.posit.co/) apps, as we no
longer operate under the single page app paradigm. Please read the
“Design Pattern” of this README for more info.

## Installation

You can install the dev version of
[brochure](https://github.com/colinfay/brochure) with:
...
(the path/endpoint where the page is available), a
[shiny](https://shiny.posit.co/) UI, a `server` function, and the HTTP
...
The matched values are read with
[`get_keys()`](https://github.com/ColinFay/brochure/reference/get_keys.md):
with no argument inside a page `server`, and with the `request` the page
`ui` receives when you need them in the UI.
...
**IMPORTANT NOTE** on top of
[`page()`](https://github.com/ColinFay/brochure/reference/page.md)s and
[`redirect()`](https://github.com/ColinFay/brochure/reference/redirect.md)s,
`...` accepts tags, tag lists, html dependencies and strings, and
injects them **as is** into every page. That is how you add something to
all your pages at once — a `<script>`, a favicon, the resources of a
[golem](https://thinkr-open.github.io/golem/) app. It also means a stray
value ends up rendered:
...
3.  The request is matched against your
...
Note that you can get them from the server with
[`get_cookies()`](https://github.com/ColinFay/brochure/reference/cookies-server-side.md),
and parse the cookie string using `parse_cookie_string`.

Give
[`remove_cookie()`](https://github.com/ColinFay/brochure/reference/cookie-middleware.md)
the same `path` and `domain` you gave
[`set_cookie()`](https://github.com/ColinFay/brochure/reference/cookie-middleware.md):
a browser only replaces a cookie when all three match, so a deletion
sent without them quietly leaves the cookie in place. This shows up once
the app is served under a prefix, where the default path is the mount
rather than `/`.
...
building a [shiny](https://shiny.posit.co/) app that works as a single
page application. This is no longer the case in
...
You can set up a [brochure](https://github.com/colinfay/brochure) based
app with [golem](https://thinkr-open.github.io/golem/) using the
[`brochure::golem_hook()`](https://github.com/ColinFay/brochure/reference/golem_hook.md)
function.
...
You can also use the module_template function to create a
[brochure](https://github.com/colinfay/brochure) module :
...
To adapt your [golem](https://thinkr-open.github.io/golem/) based
application to [brochure](https://github.com/colinfay/brochure), here
are the two steps to follow:
...
[brochure](https://github.com/colinfay/brochure) does, as they both
