...
We'll also build an `healthcheck` endpoint that simply returns a 200 `httpResponse`..
...
This is different from what you usually do when you are building a `{shiny}` app, that works as a single page application, which is no longer the case in  `{brochure}`.
