...
An `_handler` is a function that takes as argument(s):
- `req`, the request object,  for `req_handlers` 
- `res`, the response object, & `req` for `res_handlers`

`req_handlers` __must__ return `req` & `res_handlers` __must__ return `res`, potentially modified. 
...
This early `httpResponse` will not be passed to the `res_handlers`.
