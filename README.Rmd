...
Route matching can be done through various approach:

- `href` is a string, and the route is matched exactly
- `href` is a regular expression, and the route is matched against it
- `href` is a string patterns (`/page/:id/:name`). The route is matched against it, and the parameters are available in the `get_params()` function from the server side.
...
brochureApp(
  ...
  page(
    ...
    ui = fluidPage(
      h1("This is my second page"),
      tags$p("It will match any path after /page2/")
    )
  ),
  page(
    href = "/page3/:id",
    ui = fluidPage(
      h1("This is my third page"),
      tags$p("It will match any path with that pattern: /page3/:colin, /page3/:john, etc., and the parameter will be available in the server function"),
      uiOutput("id")
    ),
    server = function(input, output, session) {
      cli::cat_rule("page 3 server")
      output$id <- renderUI({
        paste(
          "You used the id",
          get_params()$id
...
browseURL("http://127.0.0.1:7407/page3/ip?id=1")
```


When using paths, you should be aware of the fact that:

- `/page2/` and `/page2` are treated as different paths.
- The first path found is matched. For example, if you have a page with `/.*` and another one with `/[a-z]*`, the first one will always be matched.

The routing path is heavily inspired by (if not copied from) how it works with ExpressJS <https://expressjs.com/en/guide/routing.html>.
Please read the [routing]() vignette for more details.
