...
You can add req_handlers and res_handlers at the app & at page level. 
A req_handlers/res_handlers is a function that takes one or two argument(s), `req`  for req_handlers or `req` and `res` for res_handlers, and return `req` (req_handlers) & `res` (res_handlers), potentially modified. 
Each page & the app have a `req_handlers` and `res_handlers` parameters, that can take a list of functions.
...
res_handlers can be interesting to set cookies
