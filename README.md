...
ui <- function(request){
  brochure(
    ...
    page(
      ...
      res_handlers = list(
        function(res, req){
          res$headers$`Set-Cookie` <- "logged=12; HttpOnly;"
          res
        }
      )
    ), 
    page(
      ...
      ui = tagList(
        h1("Bye"),
        verbatimTextOutput("cookie3")
      ), 
      res_handlers = list( 
        function(res, req){
          res$headers$`Set-Cookie` <- "logged=12; Expires=Wed, 21 Oct 1950 07:28:00 GMT"
          res
        }
      )
    )
